...
def test_explanation(explanation: str, target_class: int, x: torch.Tensor, y: torch.Tensor,
                     ...
                     inequalities=False, return_num_min_terms = False) \
    ...
    if return_num_min_terms:
        return accuracy, torch.stack(local_predictions, dim=0).sum(dim=0) > 0 if give_local else predictions, torch.stack(local_predictions, dim=0).sum(dim=0)
    return accuracy, torch.stack(local_predictions, dim=0).sum(dim=0) > 0 if give_local else predictions
