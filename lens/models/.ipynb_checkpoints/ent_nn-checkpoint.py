...
class XEntNN(BaseClassifier, BaseXModel):
    """
    ...
    def __init__(self, n_classes: int, n_features: int, hidden_neurons: list, loss: torch.nn.modules.loss,
                 dropout_rate: 0.0 = False, l1_weight: float = 1e-4, temperature: float = 0.7, device: torch.device = torch.device('cpu'),
                 name: str = "ent_net.pth"):
        ...
        for i in range(len(hidden_neurons) + 1):
            ...
            if i == 0:
                layers.append(te.nn.EntropyLinear(input_nodes, output_nodes, n_classes=n_classes, temperature=temperature))
            else:
                layers.append(torch.nn.Linear(input_nodes, output_nodes))
            if i != len(hidden_neurons) and i != 0:
                layers.extend([
                    torch.nn.LeakyReLU(),
                ])
    ...
    def get_loss(self, output: torch.Tensor, target: torch.Tensor, epoch: int = None, epochs: int = None)\
        ...
        return output_loss + l1_weight * te.nn.functional.entropy_logic_loss(self.model)
...
,
    ...
    def get_global_explanation(self, x, y, target_class: int, top_k_explanations: int = None,
        ...
        if self.explanations[target_class] != "":
            ...
            x_explain = x
            y_explain = y
            mask_explain = torch.ones(x_explain.shape[0]).to(x_explain.device).bool()
            if x_val is not None:
                x_explain = torch.vstack([x_explain, x_val])
                y_explain = torch.nn.functional.one_hot(torch.hstack([y_explain, y_val]))
                mask_explain = torch.hstack([mask_explain, torch.zeros(x_val.shape[0])]).bool()
            explanation, _ = te.logic.nn.entropy.explain_class(self.model, x_explain, y_explain, mask_explain, 
                                          ~mask_explain, target_class, topk_explanations=top_k_explanations, concept_names=concept_names, simplify=simplify, y_threshold=thr)
