...
class PrecisionScore(Metric):
    """
    Precision score computed on the predictions of a model and the actual labels. Useful for Multi-label classification.
    """
    def __init__(self, average='macro'):
        self.average = average
    ...
    def __call__(self, outputs: torch.Tensor, targets: torch.Tensor) -> float:
        ...
        # Multi-label or Binary classification
            ...
            targets = targets > 0.5
        targets = targets.cpu().numpy()
        precision_val = precision_score(discrete_output, targets, average=self.average, zero_division=0) * 100
...
class RecallScore(Metric):
    ...
    def __call__(self, outputs: torch.Tensor, targets: torch.Tensor) -> float:
        ...
        # Multi-label or Binary classification
            ...
            targets = targets > 0.5
        targets = targets.cpu().numpy()
        recall_val = recall_score(discrete_output, targets, average=self.average, zero_division=0) * 100
        return recall_val
